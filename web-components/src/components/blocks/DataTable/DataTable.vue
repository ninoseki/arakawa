...
const cols: ComputedRef<Col[]> = computed(() => {
  ...
  return colNames.map((n: string) => {
    const schemaField = schema.value && schema.value.find((f: any) => f.name === n)
    ...
    const schemaType = schemaField && schemaField.type ? schemaField.type : 'string'
...
const runQuery = () => {
  ...
  const guessQueryColumnType = (col: string, queryData: any[]) => {
    ...
    const firstNRows: any[] = queryData.slice(0, rowLimit).map((r) => r[col])
    const rowTypes: string[] = firstNRows.map(guessValueType)

    const isAllTypesEqual = rowTypes.every((rowType) => rowType === rowTypes[0])

    if (isAllTypesEqual && rowTypes[0] !== 'string') {
      // If all row types are equal and non-string then return the row type
      return rowTypes[0]
    }
    if (new Set(firstNRows).size <= DISTINCT_CATEGORIES_LIMIT) {
...
<template>
  <div
    ...
    :class="['rounded shadow w-full', { 'h-full flex flex-col': singleBlockEmbed }]"
  >
    <table-header
    ...
    <div v-if="previewMode" class="w-full flex justify-center">
      <ar-button dataCy="button-load-dataset" @click="emit('load-full')" icon="fa fa-table">
