...
def _get_environment() -> PythonEnvironment:
    ...
    if {"CODESPACES", "JUPYTERLAB_PATH", "VSCODE_CWD"}.issubset(set(os.environ)):
        return CodespacesVSCodeJupyterEnvironment()
    if {"CODESPACES", "JUPYTERLAB_PATH", "JPY_PARENT_PID"}.issubset(set(os.environ)):
        return CodespacesJupyterLabEnvironment()
    if is_zqm_interactive_shell() and "PAPERMILL_OUTPUT_PATH" in get_ipython_user_ns():
        return PapermillEnvironment()
    if is_zqm_interactive_shell() and "VSCODE_PID" in os.environ:
        return VSCodeJupyterEnvironment()
    if is_zqm_interactive_shell() and {"JPY_SESSION_NAME", "JPY_PARENT_PID"}.issubset(
        set(os.environ)
    ):
        return JupyterLabEnvironment()
    if is_zqm_interactive_shell() and "JPY_PARENT_PID" in os.environ:
        return JupyterNotebookEnvironment()
    if is_zqm_interactive_shell():
        return UnsupportedNotebookEnvironment()
    if "PYCHARM_HOSTED" in os.environ:
        return PyCharmEnvironment()
    if "VSCODE_PID" in os.environ:
        return VSCodeEnvironment()
    if is_terminal_interactive_shell():
...
class VSCodeJupyterEnvironment(JupyterEnvironment):
    ...
    def _get_notebook_path(self) -> Path:
        ...
        return Path(str(sans_suffix).split("-jvsc-")[0]).with_suffix(suffix)
...
class ColabEnvironment(IPythonZMQEnvironment):
    ...
    def get_notebook_json(self) -> dict:
        ...
        downloaded.seek(0)
        return json.loads(downloaded.read().decode("utf-8"))
