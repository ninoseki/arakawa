...
from arakawa.common.df_processor import process_df
...
def parse_dates(data: pd.DataFrame, force_utc: bool = False):
    ...
    def try_to_datetime(ser: pd.Series) -> datetime.datetime:
        return pd.to_datetime(
            ser, infer_datetime_format=True, errors="ignore", utc=force_utc
        )
...
def _check_dates_parsed(df: pd.DataFrame, date_columns: List[str]):
    assert set(df.select_dtypes("datetime").columns) == set(date_columns)


def _check_timedelta_parsed(df: pd.DataFrame, timedelta_columns: List[str]):
    assert set(df.select_dtypes("timedelta").columns) == set(timedelta_columns)
...
def test_parse_dates_nulls():
    # NOTE - using just Z, not .000Z means pandas doesn't infer timezone,
    #        so naive datetime object
    data = convert_csv_pd(
        """
        timestamp,value
