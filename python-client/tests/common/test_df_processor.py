...
def test_convert_axis():
    ...
    data = {
        "Jan": pd.DataFrame(
            columns=["Market", "Units Sold", "New Customers"], data=[["East", 14, 1]]
        ).set_index("Market"),
        "Feb": pd.DataFrame(
            columns=["Market", "Units Sold", "New Customers"], data=[["East", 45, 6]]
        ).set_index("Market"),
    }
...
def test_parse_categories_roundtrip(tmp_path: Path):
    ...
    pd.testing.assert_frame_equal(
        df1, df2, check_categorical=False, check_dtype=False, check_column_type=False
    )
...
def test_downcast_numbers():
    ...
    data = pd.DataFrame(
        [
            [1, 1, 1, 1, 1, 1, 1, 1],
            [100, 1000, int(1e6), int(1e12), -100, -1000, int(-1e6), int(-1e12)],
        ],
        columns=[
            "num1_col",
            "num2_col",
    ...
    assert [str(x) for x in data.dtypes] == [
        "uint8",
        "uint16",
        "uint32",
        "uint64",
...
def test_to_str():
    ...
    assert isinstance(data2["cat_obj_col"].cat.categories[0], str)
...
def test_e2e_df_processing(tmp_path: Path):
    ...
    df = pd.DataFrame(
        {
            ...
            "int64_col": [*list(range(29)), int(-10000000000.0)],
    ...
    _test_df(
        ...
        [
            ...
            "Int64",
            "Int8",
            "Float64" if sys.platform == "win32" else "Int64",
            "Float64",
        ],
    )
    ...
    df = pd.DataFrame(
        {
            ...
            "int_na_col": [*list(range(30)), np.nan, np.nan],
